...
--###### --- 11gR1 compatible
--# - Had to change calculation formula for backward compatiblity to 11gR1
--# - Did not have much time to check .. so use it and let me know :)
--############################
...
fv_ta_obj(fv_ta_obj.count) := ty_obj( '+Inst-------+CPUIDL%--IO%-USR%--SYS%+--Tprse/s--+Hprse/s+--PhyWIO/s-+-PhyWMB/s-+--PhyRIO/s--+-PhyRMB/s-+-SessLIO/s--+---Exec/s-+RedoMB/s+Commit/s+-ExSSMB/s-+-ExSIMB/s+-ExFCRh/s+') ;
   ...
   for i in ( select distinct inst_id from table(fv_lastsample ) order by inst_id ) loop     -- processing order by inst_id. Pls. note : no restriction on no. of instances
            ...
            fv_ta_obj(fv_ta_obj.count) := ty_obj(
              ...
              lpad(ii.redomb,8,' ')||'|'||
              lpad(ii.ccommit,8,' ')||'|'||
              lpad( nvl(ii.ExSSMB,0),10,' ')||'|'||
              lpad( nvl(ii.ExSIMB,0),9,' ')||'|'||
              lpad( nvl(ii.ExFCRh,0),9,' ')||'|'
              ) ;
  ...
  -------
     ...
     fv_ta_obj(fv_ta_obj.count) := ty_obj( '                              TOTAL :'||
              ...
              lpad(vt_redo,8,' ')||','||
              lpad(vt_comt,8,' ')||','||
              lpad(nvl(vt_ExSS,0)  ,10,' ')||','||
              lpad(nvl(vt_ExSI,0)  ,9,' ')||','||
              lpad(nvl(vt_ExFC,0)  ,9,' ')||',' );
return fv_ta_obj;
exception
when others then
...
fv_sql     varchar2(2000) :=
 q'[select ty_gash(gash.Inst_id,gash.session_id,gash.sql_id,gash.sql_child_number, substr(sql_Text,1,100), null,
                                ...
                                case when session_state = 'WAITING' then gash.Time_waited else gash.wait_Time end,
                                gash.current_obj# )
                                ...
                                and  (gash.time_Waited+gash.wait_Time) > 0
...
pv_top_sql.delete;
 ...
 for ii in ( select pct,sql_id||'('||sql_child||')' sql_id from (
             select  sql_id,sql_child, round(sqltime/tottime * 100,1) pct from
               ...
               fv_sid_string := null;
               for lstring in ( select distinct sql_id,sql_child,inst_id,sid from table(fv_only_gash) where sql_id||'('||sql_child||')' = ii.sql_id
                                and rownum < 50 ) loop -- using rownum to restrict the total no. of records, should fit in fv_sd_string
               fv_sid_string := fv_sid_string||lstring.inst_id||':'||lstring.sid||', ';
               end loop;
...
v_rcnt        int := 0 ;                 -- Below section, I have to modify for 11.1 compatiblity. had to join with gv$sql, in 11.2 sql_text column directly available in gv$sql_monitor
fv_sql        varchar2(800) := q'[ select ty_gsqlm(gsm.inst_id,gsm.sql_id,
                                    (select substr(sql_text,1,100) from gv$sql gs where gs.inst_id = gsm.inst_id and gs.sql_id = gsm.sql_id and gs.sql_id is not null and rownum = 1 )
                                                    ,gsm.elapsed_time,gsm.cpu_time,gsm.concurrency_wait_time,
                                                    gsm.cluster_wait_Time,gsm.user_io_wait_time,0,gsm.px_server#)
                                    from gv$sql_monitor gsm
                                    where gsm.status = 'EXECUTING' and gsm.elapsed_time > 0
...
for i in (      select (select  to_char(to_date(round(max(elapsed_time)/1000000),'sssss'),'hh24:mi:ss') from table(pv_only_gsqlm) gsq1 where gsq1.sql_id = gsm.sql_id) maxduration,
                     (select count(distinct inst_id) from table(pv_only_gash)  sq1 where sq1.sql_id = gsm.sql_id) acrossins,
                            ...
                            ( select lpad(nvl(round(sum(cpu_time)/ nvl(sum(elapsed_time),1) *100,1),0),6,' ')||' | '||lpad(nvl(round(sum(concurrency_Wait_Time)/nvl(sum(elapsed_time),1)*100,1),0),6,' ') ||'  | '||
                                     lpad(nvl(round(sum(cluster_Wait_Time)/nvl(sum(elapsed_time),1)*100,1),0),6,' ')||'  | '||lpad(nvl(round(sum(user_io_wait_time)/nvl(sum(elapsed_time),1)*100,1),0),4,' ')||'|'||lpad(0,10,' ')||'|'   --  I had to remove physcal* due to 11.1,
...
p_sample  := pv_Sample;
      ...
      --- Printing Banner..
        ...
        pipe row ( ty_obj('Taking first sample (11gR1 compatible ).......             ')); v_tot_rec := v_tot_Rec + 1;
